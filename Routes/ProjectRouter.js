...
router.get("/deleteProject/:id", async (req, res) => {
  ...
  router.post("/editproject/:id", async (req, res) => {
    const ProjectId= req.params.id;
    try {
        const { title,description,imageContainer,  Photos,  Gallery,addMoreDetails, Feature,Featured,Area,Floors,location,Address,City,Postalcode, Parking,Video_url,location_url, Note,CurrentStatus} = req.body;
        let Projectsaved=await Project.findOneAndUpdate({_id:ProjectId},{ title,description,imageContainer,Photos,Gallery,addMoreDetails, Feature,Featured,Area,Floors,location,Address,City,Postalcode, Parking,Video_url,location_url, Note,CurrentStatus});
        res.status(200).send(Projectsaved);
    } catch (err) {
      res.status(200).send(err);
    }
